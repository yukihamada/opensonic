...
REPO="https://github.com/enablerdao/opensonic.git"
...
echo ""
echo -e "${BOLD}Soluna Installer${NC} — Mac ネットワークオーディオ"
echo "────────────────────────────────────────"
echo ""
...
info "ビルド中 (1〜2分)..."
BUILD_DIR="$TMP_DIR/opensonic/build"
cmake -S "$TMP_DIR/opensonic" -B "$BUILD_DIR" \
...
cmake --build "$BUILD_DIR" --target solunad --parallel "$(sysctl -n hw.logicalcpu)" \
...
# ── Install binary ─────────────────────────────────────────
info "インストール先: $INSTALL_DIR/solunad"
...
success "solunad をインストールしました"

# ── Install launchd service ────────────────────────────────
PLIST_SRC="$TMP_DIR/opensonic/apps/daemon/io.soluna.daemon.plist"
...
if [[ -f "$PLIST_SRC" ]]; then
    ...
    success "launchd サービスを登録しました (自動起動 ON)"
else
...
echo ""
echo "  solunad はバックグラウンドで起動しています。"
echo "  ブラウザで http://localhost:8400 を開くとダッシュボードが表示されます。"
echo ""
echo "  コマンド:"
echo "    停止:   launchctl stop  $PLIST_ID"
echo "    起動:   launchctl start $PLIST_ID"
echo "    アンインストール: launchctl bootout gui/\$UID $PLIST_ID && rm $PLIST_DST"
echo ""
