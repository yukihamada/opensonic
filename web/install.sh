...
# ── Build Soluna CoreAudio plugin ──────────────────────────
info "Soluna オーディオドライバーをビルド中..."
cmake --build "$BUILD_DIR" --target Soluna --parallel "$NPROC" \
    > "$TMP_DIR/plugin.log" 2>&1 || { tail -20 "$TMP_DIR/plugin.log"; die "ドライバービルド失敗"; }

DRIVER_SRC="$BUILD_DIR/apps/plugin/Soluna.driver"
[[ -d "$DRIVER_SRC" ]] || die "ドライバーが見つかりません: $DRIVER_SRC"
success "Soluna ドライバービルド完了"
...
# ── Install Soluna CoreAudio plugin ───────────────────────
info "Soluna オーディオドライバーをインストール中..."
echo -e "  ${YELLOW}(sudo パスワードが必要です)${NC}"
...
sleep 2
success "Soluna ドライバー → $DRIVER_DEST"
