 ...
 */
export function parseOSTPacket(data: Uint8Array): OSTPacket | null {
  if (data.length < OSTConstants.rtpHeaderSize) return null;

  // Check RTP version bits: (byte[0] & 0xC0) == 0x80
  if ((data[0] & 0xC0) !== 0x80) return null;
