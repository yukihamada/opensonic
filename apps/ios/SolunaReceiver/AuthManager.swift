...
class AuthManager: ObservableObject {
    ...
    private let keychainService = "com.soluna.SolunaReceiver"
    ...
    func requestCode(email: String) async {
        ...
        do {
            ...
            if http.statusCode == 200 {
                ...
                error = NSLocalizedString("error_rate_limit", value: "しばらく待ってから再試行してください", comment: "Rate limit error message")
            } else {
                let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
                error = json?["error"] as? String ?? NSLocalizedString("error_send_failed", value: "送信に失敗しました", comment: "Send failed error message")
            }
        } catch {
            self.error = "\(NSLocalizedString("error_network", value: "ネットワークエラー", comment: "Network error prefix")): \(error.localizedDescription)"
        }
    }
        ...
        do {
            ...
            if http.statusCode == 200,
               ...
               let token = json["token"] as? String {
                ...
                let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
                error = json?["error"] as? String ?? NSLocalizedString("error_auth_failed", value: "認証に失敗しました", comment: "Authentication failed error message")
            }
        } catch {
            self.error = "\(NSLocalizedString("error_network", value: "ネットワークエラー", comment: "Network error prefix")): \(error.localizedDescription)"
        }
    ...
    func checkUsername(_ name: String) async -> Bool {
        ...
        guard let url = URL(string: "\(relayBase)/api/auth/check-username?name=\(encoded)") else { return false }
        do {
    ...
    func setUsername(_ name: String) async -> Bool {
        ...
        guard let url = URL(string: "\(relayBase)/api/auth/set-username") else { return false }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
        ...
        do {
            ...
            if http.statusCode == 200,
               let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               let returned = json["username"] as? String {
                username = returned
                return true
            } else {
                let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
                error = json?["error"] as? String ?? NSLocalizedString("error_username_failed", value: "ユーザー名の設定に失敗しました", comment: "Username set failed error message")
                return false
            }
        } catch {
            self.error = "\(NSLocalizedString("error_network", value: "ネットワークエラー", comment: "Network error prefix")): \(error.localizedDescription)"
