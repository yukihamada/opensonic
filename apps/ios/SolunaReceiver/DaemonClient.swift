...
final class DaemonClient: ObservableObject {
    ...
    @Published private(set) var channelConnected: Bool = false
    ...
    func getChannel() {
        send(#"{"id":\#(nextId()),"command":"channel.get"}"#)
    }
    ...
    private func handleEvent(_ event: String, json: [String: Any]) {
        ...
        case "channel.changed":
            if let ch = json["channel"] as? String {
                channel = ch
    ...
    private func parse(_ text: String) {
        ...
        } else if let ch = d["channel"] as? String {
            channel = ch
            channelConnected = d["connected"] as? Bool ?? false
